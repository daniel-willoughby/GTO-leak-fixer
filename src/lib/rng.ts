// Tiny deterministic PRNG. Used so every client builds the *same* daily ladder
// from the shared UTC date string, no server round-trip, identical questions
// for everyone on a given day.

/** Hash a string to a 32-bit unsigned int (xmur3-style). */
export function hashStr(str: string): number {
  let h = 1779033703 ^ str.length
  for (let i = 0; i < str.length; i++) {
    h = Math.imul(h ^ str.charCodeAt(i), 3432918353)
    h = (h << 13) | (h >>> 19)
  }
  h = Math.imul(h ^ (h >>> 16), 2246822507)
  h = Math.imul(h ^ (h >>> 13), 3266489909)
  return (h ^= h >>> 16) >>> 0
}

/** Seeded 0..1 generator (mulberry32). Returns a function; call for each draw. */
export function mulberry32(seed: number): () => number {
  let a = seed >>> 0
  return function () {
    a |= 0
    a = (a + 0x6d2b79f5) | 0
    let t = Math.imul(a ^ (a >>> 15), 1 | a)
    t = (t + Math.imul(t ^ (t >>> 7), 61 | t)) ^ t
    return ((t ^ (t >>> 14)) >>> 0) / 4294967296
  }
}

/** Convenience: a seeded RNG straight from a string key. */
export const seededRng = (key: string): (() => number) => mulberry32(hashStr(key))

/** Pick a deterministic element from an array using a seeded rng draw. */
export const pick = <T,>(rng: () => number, arr: readonly T[]): T => arr[Math.floor(rng() * arr.length)]

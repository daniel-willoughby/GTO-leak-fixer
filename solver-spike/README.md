...
## Enlarged low-power run: ~150 boards + turn continuations (the v3 batch)

Same low-power settings as before (`set_accuracy 0.5`, `set_max_iteration 90`,
8 threads, isomorphism on) — just **5× the boards**. `BOARDS` in `solve-boards.mjs`
is now 150 (the 30 originals first, then 120 new ones generated by
`gen-board-set.mjs` across texture buckets: ace/king/queen-high dry, broadway,
two-tone connected mid + low, low rainbow, paired, trips, monotone). Each board
still emits flop + turn nodes, so this run roughly 5×'s the turn corpus the daily
challenge's turn rungs (18-19) draw from.

```bash
# regenerate the board literal (only if you want to reshuffle the set)
node solver-spike/gen-board-set.mjs > /tmp/boards.js   # paste over BOARDS

# solve — RESUMABLE: the 24 already-extracted boards are skipped, ~126 remain.
...
At ~1-3 min/board that's roughly a 3-6 hour batch; leave it running and commit
`src/data/street-nodes.json` when it's done. New boards get flop+turn only (the
app's `hasRiver()` gate hides the river-continue button where river data is
absent); river continuation stays on the original boards.

...
## 400-board run: a 4-day low-power batch (the v3 batch)

Same low-power settings as before (`set_accuracy 0.5`, `set_max_iteration 90`,
8 threads, isomorphism on). `BOARDS` in `solve-boards.mjs` is a **400-board
texture-representative sample of the 1755 strategically-distinct flops** — sized
so the batch runs ≈ **4 days** at the measured ~15 min/board (iter 90). The 30
originals come first (so their river data still lines up); the rest are canonical
representatives from `gen-all-flops.mjs`. Each board emits flop + turn nodes, so
this ~13×'s the turn corpus the daily challenge's turn rungs (18-19) draw from.

`gen-all-flops.mjs` enumerates all 1755 flop classes up to suit isomorphism and
can emit an even sample: `node solver-spike/gen-all-flops.mjs 400` (drop the arg
for the full 1755, which would take ~18 days at this quality).

```bash
# regenerate the board literal (only if you want to resize the set)
node solver-spike/gen-all-flops.mjs 400 > /tmp/boards.js   # paste over BOARDS

# solve — RESUMABLE: the 24 already-extracted boards are skipped, ~376 remain.
# Low-power, one board at a time (deletes each ~150 MB dump before the next).
node --max-old-space-size=4096 solver-spike/solve-boards.mjs   # → .street-nodes.staging.json

node solver-spike/validate.mjs    # heuristic spot-check
node solver-spike/finalize.mjs    # promote staging → src/data/street-nodes.json (+ river re-add)

npx tsc --noEmit -p tsconfig.app.json   # data-integrity check
```

Leave it running (~4 days) and commit `src/data/street-nodes.json` when it's
done. New boards get flop+turn only (the app's `hasRiver()` gate hides the
river-continue button where river data is absent); river continuation stays on
the original boards.
